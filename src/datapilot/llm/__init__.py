...
def build_provider(cfg: LLMConfig) -> LLMProvider:
    ...
    name = cfg.provider
    if name == "none":
        from datapilot.llm.null_provider import NullProvider

        return NullProvider()
    if name == "bedrock":
        from datapilot.llm.bedrock import BedrockProvider

        return BedrockProvider(cfg)
    if name == "ollama":
        from datapilot.llm.ollama import OllamaProvider

        return OllamaProvider(cfg)
    if name == "openai":
        from datapilot.llm.openai_compat import OpenAICompatProvider
